...
T = 0.7*np.sum(np.abs(s_log.flatten()))/s_log.size
s_log_thresh = np.abs(s_log)>T

T = 0.7*np.sum(np.abs(s_laplace.flatten()))/s_laplace.size
s_laplace_thresh = np.abs(s_laplace)>T

edges_canny = feature.canny(im, sigma=5)
...
ax4.imshow(maxmin_norm(np.uint(edges_canny)), cmap=plt.cm.gray)
ax4.axis('off')
ax4.set_title('Canny', fontsize=20)
fig.tight_layout()
plt.show()
