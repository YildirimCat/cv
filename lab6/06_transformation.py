...
def main():
    ...
    rotation_matrix = np.float32([[np.cos(np.pi/3),-np.sin(np.pi/3),0],[np.sin(np.pi/3),np.cos(np.pi/3),0],[0,0,1]])
    ...
    transformed = cv2.warpAffine(img, combined_matrix, (np.int32(img.shape[1]*3/4), np.int32(img.shape[0]*4/3)))
    # Do plot
    plot_cv_img(transformed)
