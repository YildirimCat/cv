...
X, Y = np.meshgrid(x, y)
gausMask2Dm = (1/(2*math.pi*sigma**2))*\
            np.exp(-1*((X**2+Y**2))/(2*sigma**2))
fig = plt.figure()
ax = fig.gca(projection='3d')
# Plot the surface.
surf = ax.plot_surface(X, Y, gausMask2Dm, cmap=cm.coolwarm,linewidth=0, \
                       antialiased=False)
...
imgplot = plt.imshow(gausMask2Dm, cmap='gray')  
plt.show()
